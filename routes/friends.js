...
router.post("/add",middlewareBodyParser,function(req,respo){
    ...
    if(!err)
     { if (rows.length > 0)
        		...
        		if (!insert_error) {
        			...
        			connection.query("insert into  user_friends( user_id ,friend_id) values(?,?)",[rows[0].user_id,req.user.user_id],function(insert_error2,insert_row2){
        				if (!insert_error2){
        					...
        					respo.send("the friend is already existed <a href='/friends'>return to the friends page </a>");
        				}
        			});
    	...
    	respo.send("Invalid User Data <a href='/friends'>return to the friends page </a>");
    }
	}
    else
